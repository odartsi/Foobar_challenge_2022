...
def solution(x,y):
    # there is a clear pattern in the triangle that it can be writen as a series.
    # expressing each element as a recursive series of the element bellow until we reach the 
    # element on the bottom row (y=1). And those elements in the bottom row are actually the results 
    # of the sum from 1 to 
    ID = int((x*(x+1))/2 + (y-1)*(x+y) - (y*(y+1))/2 +1)
    return str(ID)
